...
def safe_json_dumps(obj, **kwargs):
    ...
    return safe_json_dumps(obj, cls=DateTimeEncoder, **kwargs)
...
class UnifiedSwarmCoordinator:
    ...
    def __init__(self, config: Optional[UnifiedSwarmConfig] = None):
        ...
        # Core services
        self.llm_service = UnifiedLLMService(config={'mock_mode': True})  # Mock mode for demo
...
"""
    ...
    async def _auto_scaling_loop(self):
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                avg_load = np.mean(list(self.agent_load.values())) if self.agent_load else 0
    ...
    async def _collect_metrics(self) -> Dict[str, Any]:
        ...
        if validated_tasks:
            self.metrics.average_validation_score = np.mean([
        ...
        return {
            ...
            'avg_agent_utilization': np.mean(list(self.metrics.agent_utilization.values())),
...
async def unified_swarm_demo():
    ...
    async def demonstrate_full_system(self):
        ...
        logger.info("Phase 1: Creating diverse agent team...")
        
        # Create different agent types
        cognitive_agent = await self.create_agent(
            "analyzer",
            "cognitive",
        ...
        reflexive_agent = await self.create_agent(
            "executor", 
            "reflexive",
            capabilities=["file_operations", "code_execution"]
        )
        
        hybrid_agent = await self.create_agent(
            "coordinator",
            "hybrid", 
            capabilities=["coordination", "monitoring", "adaptation"]
        )
        
        autonomous_agent = await self.create_agent(
            "innovator",
            "autonomous",
            capabilities=["learning", "creativity", "problem_solving"]
        )
        
        # Phase 2: Create complex task
        logger.info("\nPhase 2: Creating complex multi-step task...")
        
        task_description = """
        Create a complete web application with:
        1. REST API backend with authentication
        ...
        # Phase 3: Decompose and validate
        logger.info("\nPhase 3: Decomposing and validating task...")
        
        subtasks = await self.decompose_task(task["id"])
        validated = await self.validate_task_decomposition(task["id"])
        ...
        logger.info("\nPhase 4: Executing with swarm coordination...")
        ...
        logger.info("\nPhase 5: Monitoring execution progress...")
        ...
        while task["status"] not in ["completed", "failed"]:
            ...
            # Check task status
            task = self.task_registry.get(task["id"])
        
        # Phase 6: Collect results
        logger.info("\nPhase 6: Collecting and analyzing results...")
        
        results = await self.get_task_results(task["id"])
        metrics = await self.get_swarm_metrics()
        
        logger.info("=== DEMONSTRATION COMPLETE ===")
        logger.info(f"Final status: {task['status']}")
        logger.info(f"Total subtasks: {len(results.get('subtasks', []))}")
...
def main():
    ...
    parser.add_argument("--mode", choices=["demo", "server", "test", "full"], default="demo")
